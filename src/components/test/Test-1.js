...
useEffect(()=>setState(props.user),[])
  return (
    !state ?<Fade><h1>Loading...</h1></Fade>:
    ...
    <Roll cascade delay={400} duration={1500}><img alt="here" src="https://cdn.clipart.email/48ad6b78b3ced0d25248c112a5d62441_pixel-dividers-tumblr_500-442.png"/></Roll>  
      <h1 style={{fontFamily:" 'Righteous', cursive",color:'#D86E20',fontSize:'2rem'}}>My Review  <button style={{background:"yellow",color:"black",padding:"10px"}} onClick={()=>window.location.reload()}>Update List</button></h1>
        ...
        {props.user.length === state.length &&
          props.user.map((x, i) => (
